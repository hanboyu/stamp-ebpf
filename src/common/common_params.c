                        ...
                        struct config *cfg, const char *doc)
{
	...
	while ((opt = getopt_long(argc, argv, "hd:r:L:R:ASNFU:MQ:czpqo:",
				  long_options, &longindex)) != -1) {
		switch (opt) {
		case 'o': /*--out-file */
			printf("parsing output file\n");
			if (strlen(optarg) >= sizeof(cfg->out_file)) {
				fprintf(stderr, "ERR: --out-file path too long\n");
				goto error;
			}
			dest  = (char *)&cfg->out_file;
			strncpy(dest, optarg, sizeof(cfg->out_file));
			break;
		case 't': /* --duration */
